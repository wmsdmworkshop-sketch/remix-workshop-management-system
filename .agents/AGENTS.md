...
- **⚠ CORRECTED 2026-09-16 — the rc1 profile does NOT actually run in production.** Production *invokes* `npm run build:rc1`, but `.dockerignore` **and** `.gcloudignore` both exclude `.env` and `.env.*`, and `.env.rc1` — the only file setting `VITE_WORKFORCE_PROFILE=rc1` — is gitignored as well, so the flag never reaches the build. `VITE_WORKFORCE_PROFILE` is `undefined`, every `=== "rc1"` check folds to `false`, and esbuild deletes those branches as dead code. Verified in the **served artifact**: `/assets/index-dw5hAiI8.js` contains **zero** `"rc1"` literals and no `excludedTabs` array. The net effect today is *permissive* — nothing is hidden from any role, `developer` and `admin` included. See `docs/CHANGELOG.md` v1.1.0-rc.14, "Defect 2".
  - The paragraph that stood here said production builds with the rc1 profile and that `excludedTabs` hides tab ids from **everyone, including `developer`**. **That described intent, not behaviour.** It is what led the 2026-09-15/16 investigation to publish a wrong claim that `developer` was losing six tabs in production; the claim had to be retracted the same day. **Lesson: for "is this feature on in production?" on a bundled SPA, fetch `/assets/index-*.js` and grep it — constant-folding makes source-truth and artifact-truth diverge, invisibly to review.**
  - `src/App.tsx` now has a **single** definition of this gating, `isRc1TabHidden()`, replacing the two divergent `excludedTabs` occurrences (rc.14, `5e07992`). Because the profile is inert, that consolidation is correct hygiene but a **no-op in production**.
  - **Open owner decision, deliberately not made silently:** either pass `VITE_WORKFORCE_PROFILE` through a Docker `ARG`/`ENV` or a Cloud Build substitution so it survives the context exclusion, **or** accept rc1 is inert and delete the dead checks. Enabling it would **hide** six tabs in production. `.env.*` must stay dockerignored either way — excluding env files from the image is the right instinct; `.env.rc1` merely happens to carry no secret.
...
- **Rule 11 (Exact Credential & Identity Binding)**: When a credential is provided for a specific user identifier, the agent MUST bind the password strictly and exclusively to that exact user identifier. Never substitute, mix, or alias usernames. Credentials themselves live only in the environment / secret manager — never write a real password into source, docs, or this file.
- **Rule 12 (Absolute Transparency & Feasibility-First Research — Zero User Spinning)**: Agents MUST thoroughly research and prove technical feasibility before claiming any integration, automation, or live data stream is functional. If an external integration is blocked (e.g., by proprietary JS architectures, session locking, missing OEM API keys, or 401s), the agent MUST immediately, directly, and plainly state that it is not possible right now. Agents must NEVER spin the user with technical excuses, speculative trials, or false hopes, nor waste the user's valuable time.
