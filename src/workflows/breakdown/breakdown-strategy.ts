...
export class BreakdownWorkflowStrategy extends BaseWorkflowStrategy {
  constructor(private provider: ExternalProgramProvider<BreakdownIncident>) {
    super();
  }

  getWorkflowType(): string {
    return "BREAKDOWN_ROADSIDE_ASSISTANCE";
  }
  ...
  async onBeforeTransition(
    ...
    if (command.target_state === "ASSIGNED") {
      const payload: BreakdownIncident = command.payload;
      if (!payload) {
        return failResult("Missing Breakdown payload", context);
      }
      
      const submitRes = await this.provider.submit(payload);
      if (submitRes.success && submitRes.data) {
        businessCase.references.push({
          entity_type: "BREAKDOWN_OEM_REF",
          entity_id: submitRes.data.reference,
          relationship: "tracked_incident"
        });
      }
    }
