...
export default function QuerySearch({ aiModeEnabled = true }: QuerySearchProps) {
  // Recording state
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-8 bg-slate-50 min-h-screen">
      ...
      <div>
        <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight flex items-center gap-2">
          ...
          {aiModeEnabled ? "Multimedia Query & OCR Part Search" : "Manual Stock Inventory Lookup"}
        </h1>
        <p className="text-slate-500 mt-1">
          {aiModeEnabled 
            ...
            : "Search parts inventory manually by typing part numbers or part names."}
        </p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Left Side: Input options Panel */}
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-6">
          <h2 className="text-xl font-bold text-slate-800 border-b border-slate-100 pb-3">{aiModeEnabled ? "Input Options Panel" : "Manual Part Lookup"}</h2>
          ...
          {aiModeEnabled ? (
            ...
            <div className="bg-slate-50 p-5 rounded-xl border border-slate-200 space-y-4">
              <h3 className="font-bold text-slate-700 flex items-center gap-2">
                <Search className="w-5 h-5 text-indigo-600" /> Manual Stock Lookup
              </h3>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <input
                    type="text"
                    placeholder="Enter Part Number or Part Name (e.g. Brake Pads)..."
                    className="w-full bg-white border border-slate-250 rounded-lg p-2.5 pl-9 text-xs font-bold focus:ring-1 focus:ring-indigo-500 focus:outline-hidden text-slate-800"
                    onChange={(e) => searchPartInDb(e.target.value)}
                  />
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-6">
          <h2 className="text-xl font-bold text-slate-800 border-b border-slate-100 pb-3">{aiModeEnabled ? "OCR Extracted Parts" : "Stock Inventory Query"}</h2>
          ...
          {ocrLoading ? (
            ...
            <div className="space-y-6">
              ...
              <div className="border-t border-slate-100 pt-6 space-y-4">
                ...
                {!selectedPart && (
                  <p className="text-slate-400 text-sm italic">
                    {aiModeEnabled ? "Select or click an extracted chip above to search inside parts inventory." : "Type a query in the Manual Stock Lookup box to view matching parts."}
                  </p>
                )}
              </div>
            </div>
