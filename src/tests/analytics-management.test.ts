...
async function runTests() {
  ...
  if (failed > 0) {
    ...
    // Exit explicitly: the harness leaves async handles open, so without this
    // Node waits on the event loop and the legacy test runner's per-file
    // timeout kills it before it's counted as passed.
    process.exit(0);
  }
}
